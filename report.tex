...
本次最终代码采用阻塞双边通信：根进程通过 \texttt{MPI\_Bcast} 同步矩阵状态，通过 \texttt{MPI\_Send}/\texttt{MPI\_Recv} 下发任务和回收结果，并使用 \texttt{MPI\_ANY\_SOURCE} 实现动态任务领取。需要说明的是，本文没有把 \texttt{MPI\_Isend}/\texttt{MPI\_Irecv}、RMA 单边通信分别实现为独立版本，因此不把它们写成“实测加速对比”。本节的比较方式是：以已经实测的阻塞 MPI 规模扩展数据、x86 线程并行机会数据和通信量估算，判断这些方法在本 SVD-GKH 场景中的潜在收益和风险。表\ref{tab:mpi-methods} 给出具体数据依据。
...
\midrule
阻塞双边通信 &
已实现并完成 41 组服务器实验，全部 \texttt{通过: 5 / 5}。规模扩展中 $np=4$ 的 GKH 平均耗时为 15975.63 ms，加速比 1.45；$np=32$ 的 GKH 平均耗时退化到 67490.93 ms，加速比 0.34，标准差 36518.19 ms。 &
阻塞通信正确性最好，也能在 $np=4$ 取得有限收益；但高进程数下通信、等待和节点波动会迅速吞掉收益。\\
\midrule
非阻塞双边通信 &
x86 本地随机 $512\times512$ 的可并行轮次只有 $8/749=1.07\%$，最大 ready task 只有 2；同一输入下 pthread4 和 OpenMP4 加速比分别只有 1.00 和 1.01。相反，4 个 $128$ 块样例的可并行轮次为 $252/253=99.60\%$，pthread4/OpenMP4 可达到 3.13/3.15 倍。 &
非阻塞通信只有在 ready task 足够多时才有较大重叠空间。普通随机矩阵中可重叠计算窗口太小，\texttt{MPI\_Isend}/\texttt{MPI\_Irecv} 预计只能隐藏少量回传等待；块结构输入更值得尝试。\\
\midrule
单边通信 RMA &
对 $1000\times1000$ 主样例，$U+B+V$ 完整矩阵约 24 MB；阻塞方案在 $np=32$ 时单轮完整广播量估计为 744 MB。随机矩阵又只有 1.07\% 的可并行轮次，说明大多数轮次没有足够 worker 可以共享窗口。 &
RMA 不能自动减少需要同步的数据量；若仍同步完整矩阵，通信量瓶颈仍在。只有改成活动块相关列的窗口，并解决 $U,V$ 列更新一致性问题后，单边通信才可能有收益。\\
\midrule
MPI 自身多线程支持 &
本地 x86 实测显示：随机 $512\times512$ 下 pthread4/OpenMP4 基本不加速；4 个 $128$ 块结构下，pthread4 为 360.63 ms、OpenMP4 为 359.08 ms，分别相对 serial 的 1130.35 ms 达到 3.13/3.15 倍。块结构中 worker 最大/最小工作量比例仅 1.08:1。 &
混合并行是更有数据支撑的后续方向：节点间减少 MPI rank，节点内使用线程池处理活动块，可减少矩阵副本和广播参与者，同时保留块结构下的线程级收益。\\
\bottomrule
\end{tabular}
\caption{不同 MPI 编程方法的数据支撑与适用性判断}
...
从当前 41 组服务器数据和表\ref{tab:mpi-comm-evidence} 看，阻塞 MPI 的收益与通信量之间存在明显转折：从 $np=1$ 到 $np=4$，每轮完整矩阵广播量从 0 MB 增至 72 MB，但 GKH 平均耗时从 23147.60 ms 降至 15975.63 ms，说明此时仍有足够任务收益抵消通信代价；继续增至 $np=8$ 和 $np=16$ 后，广播量估计增至 168 MB 和 360 MB，GKH 加速比分别下降到 1.34 和 1.21；到 $np=32$ 时，单轮广播量估计达到 744 MB，GKH 平均耗时反而退化到 67490.93 ms，且标准差达到 36518.19 ms。这个数据趋势说明，高进程数下的主要问题不是 \texttt{MPI\_Send}/\texttt{MPI\_Recv} 是否写成非阻塞形式，而是需要同步的数据量和可并行任务数之间已经失衡。

因此，最值得继续尝试的不是盲目引入更复杂的通信模型，而是混合并行：节点间使用较少 MPI rank，节点内复用已有 pthread/OpenMP 活动块任务池。这个判断同样有本地实测支撑。图\ref{fig:parallel-opportunity} 中，普通随机矩阵只有 1.07\% 的可并行轮次，pthread4/OpenMP4 都约为 1.0 倍；而块结构样例有 99.60\% 的可并行轮次，pthread4/OpenMP4 可达到 3.13/3.15 倍。这说明非阻塞通信和单边通信在普通随机矩阵上能重叠的计算窗口很小，更适合作为有块结构输入或混合并行方案上的后续优化分支。
